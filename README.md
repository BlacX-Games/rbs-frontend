...
| `npm run build`                             | `vite build` then `tsc -b` — in that order, see below              |
...
`build` runs Vite _before_ TypeScript because `src/routeTree.gen.ts` is gitignored and generated by a Vite plugin (see `src/router.tsx`). In a clean checkout — every CI and Vercel build — a leading `tsc -b` has no route tree to compile against and fails. For the same reason `npm run typecheck` on its own needs one `npm run dev` or `npm run build` first.
